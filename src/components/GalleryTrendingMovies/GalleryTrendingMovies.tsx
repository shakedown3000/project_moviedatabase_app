  ...
  return (
    <div className="flex overflow-x-auto space-x-4 py-4 ">
      {movies.slice(0, 4).map((movie) => (
        <div
          key={movie.id}
          className="flex-shrink-0 w-full h-64 relative bg-cover bg-top rounded-3xl"
          style={{
            backgroundImage: `url(https://image.tmdb.org/t/p/w500${movie.poster_path})`,
          }}>
          <Link to={`/detail/${movie.id}`} className="absolute inset-0 ">
            <div className="border-2 flex flex-col justify-end items-center h-full bg-gradient-to-t from-black via-transparent to-transparent rounded-3xl pb-4 ">
